...
@given(
    parsers.parse(
        "define a PVC with access mode {access_modes} and volume mode {volume_mode}"
    )
)
...
def volumesnapshot_should_be_bound(
    dynamic_client: DynamicClient, pvc: PersistentVolumeClaim
):
    volume_snapshot = VolumeSnapshot.get(
        pvc.namespace, f"snapshot-{pvc.name}", dynamic_client
    )
...
def volumesnapshot_should_not_exist(
    dynamic_client: DynamicClient, pvc: PersistentVolumeClaim
):
