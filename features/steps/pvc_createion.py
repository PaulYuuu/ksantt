...
@given("define a PVC with accessModes {access_modes} and volumeMode {volume_mode}")
...
def step_create_pvc(context: Context):
    ...
    context.pvc.create()
    context.pvc.logger.info(
        f"Created PVC '{context.pvc.name}' in namespace '{context.pvc.namespace}'."
    )
...
def step_pvc_should_be_bound(context: Context):
    ...
    context.pvc.logger.info(
        f"PVC '{context.pvc.name}' is Bound in namespace '{context.pvc.namespace}'."
    )
...
def step_delete_pvc(context: Context):
    ...
    if context.pvc.exists:
        ...
        context.pvc.logger.info(
            f"Deleted PVC '{context.pvc.name}' from namespace '{context.pvc.namespace}'."
        )
    else:
...
def step_pvc_should_not_exist(context: Context):
    ...
    assert not context.pvc.exists, (
        f"PVC '{context.pvc.name}' still exists after deletion."
    )
    context.pvc.logger.info(
        f"Confirmed PVC '{context.pvc.name}' no longer exists in namespace '{context.pvc.namespace}'."
    )
